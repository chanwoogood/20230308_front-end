/*
   객체(Object)
       ...
       - 데이터를 사람의 관점으로 나타내는 방식.
       - key(속성이름)-value(속성값)로 속성을 나타낸다.
       - {} 기호를 통해서 작성하는 방식 : 객체 리터럴.
       - 상태(프로퍼티)와 행위(매서드)를 통해 동작할 수 있다.
...
let user = {
  ...
  hello: function () {
    ...
    // 템플릿 리터럴 : ``(백틱) 안에서 ${JS 표현식}을 사용하면 JS값을 문자열 안에 포함시킬 수 있다.
...
console.log(obj.name.lastName); // obj.name이 Undefined이기 때문에 오류 발생.
