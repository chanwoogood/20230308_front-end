...
// 전달 받은 숫자 n만큼 코드(콜백함수)를 반복 실행하는 repeat함수 만들기.
...
// 안녕하세요 저는 25세 Sungjin입니다. 의 형식으로 배열의 각 요소를 모두 출력.
let users = [
  { name: "Chanwoo", age: 26 },
  { name: "Sungjin", age: 32 },
  { name: "Jinsung", age: 35 },
...
/*
  Array.prototype.filter()
      ...
      - 특정 조건을 만족하는 요소만을 모아서 새로운 배열로 반환된다.
        => 인자로 전달된 콜백함수의 return값이 true나 false여야된다. 즉, 조건식이어야한다.
...
/*
  Array.prototype.map()
      - forEach와 마찬가지로 배열을 순회하며 실행한다.
      - 요소의 값을 통해서 새로운 값을 만들어서 배열로 모아서 반환된다.
         => 콜백함수의 return값이 새로운 배열이 요소로 모여서 반환된다.
...
/*
  Array.prototype.reduce()
     - 결과를 누적시키는 누산기를 사용한다.
     - 콜백함수의 첫번째 매개변수에는 이전 회차의 값, 두번째 인자는 현재 회차의 값.
...
// users => age가 30이상인 객체의 이름만 oldUserNames 배열로 모으기
...
/*
  유사배열 : 배열처럼 key값이 인덱스이고, length 프로퍼티를 갖는 객체.
     - 이름처럼 배열과 유사하지만 배열은 아니기 때문에 배열의 다른 함수들을 사용하는 것이 불가능.
        => HTMLCollection, NodeList 등... 갱장히많다.
*/
let arrayLike = {
  0: "Son",
  1: "Chanwoo",
...
// arrayLike.forEach((item) => console.log(item)); 배열이 아니기 때문에 forEach 함수가 없다고 나온다.
/*
  Array.from()
